<%@ Page Title="" Language="C#" MasterPageFile="~/Common/Template/MainFrame.Master"
    AutoEventWireup="true" CodeBehind="MergeInvoiceAdjustAPT.aspx.cs" Inherits="KN.Web.Management.Manage.MergeInvoiceAdjustAPT" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphContent" runat="server">
    ...
    <!--        //
        ...
        function fnSelectCheckValidate(strText) {
            var ddlItemCd = document.getElementById("<%=ddlItemCd.ClientID%>");

            if (trim(ddlItemCd.value) == "") {
                alert(strText);
                document.getElementById("<%=ddlItemCd.ClientID%>").focus();
                return false;
            }
            
            var txtPeriod = document.getElementById("<%=txtPeriod.ClientID%>");

            if (trim(txtPeriod.value) == "") {
                alert(strText);
            ...
            if (parseInt(trim(txtSearchDt.value.replace('-',''))) == parseInt(trim(txtPeriod.value.replace('-','')))) {
                alert("Check period or paid date !");
                document.getElementById("<%=txtSearchDt.ClientID%>").focus();
                return false;
            }
        ...
        function fnOccupantList(refPrintNo,strBilCd, strPayDt) {        
            CloseLoading();
            document.getElementById("<%=hfsendParam.ClientID%>").value = refPrintNo;
            document.getElementById("<%=hfBillCd.ClientID%>").value = strBilCd;

            window.open("/Common/RdPopup/RDPopupAdjustmentInvoiceAPT.aspx?Datum0=" + refPrintNo + "&Datum1=" + strBilCd + "&Datum2=" + strPayDt, "Reciept", "status=no, resizable=no, width=740, height=800, left=100,top=100, scrollbars=no, menubar=no, toolbar=no, location=no", false);
...
<fieldset class="sh-field5 MrgB10" >
    ...
    <ul class="sf5-ag MrgL30">
        ...
        <li><b><asp:Literal ID="Literal3" runat="server" Text="Period :"></asp:Literal></b></li>
        <li><asp:TextBox ID="txtPeriod" CssClass="grBg bgType2" MaxLength="10" Width="70px" runat="server"></asp:TextBox></li>
     ...
     <ul class="sf5-ag MrgL30 bgimgN">
        ...
        <li><b><asp:Literal ID="Literal4" runat="server" Text="Invoice No (For export only) :"></asp:Literal></b></li>
        <li><asp:TextBox ID="txtInvoice" CssClass="grBg bgType2" MaxLength="10" Width="70px" runat="server" Visible="True"></asp:TextBox></li>               
