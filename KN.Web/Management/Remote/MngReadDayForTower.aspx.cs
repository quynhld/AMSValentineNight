...
{
    public partial class MngReadDayForTower : BasePage
    {
        ...
        protected void LoadData()
        {
            ...
            // KN_USP_MNG_SELECT_UTILCHARGEINFO_S02
            dsReturn = RemoteMngBlo.SpreadUtilDayUseChargeList(CommValue.BOARD_VALUE_ROOMSIZE, Int32.Parse(hfCurrentPage.Value), txtHfRentCd.Text, txtHfChargeTy.Text, ddlYear.Text, ddlMonth.Text, intSearchFloor, txtSearchRoom.Text);
        ...
        protected void lvDayChargeList_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            if (e.Item.ItemType.Equals(ListViewItemType.DataItem))
            {
                ...
                if (!string.IsNullOrEmpty(drView["EnergyDay"].ToString()))
                {
                    ...
                    ltDayList.Text = TextLib.MakeDateEightDigit(strDate);
                }
        ...
        protected void lnkbtnModify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect(Master.PAGE_MODIFY + "?" + Master.PARAM_DATA1 + "=" + txtHfRentCd.Text + "&" + Master.PARAM_DATA2 + "=" + txtHfChargeTy.Text + "&" + Master.PARAM_DATA3 + "=" + ddlYear.Text + "&" + Master.PARAM_DATA4 + "=" + ddlMonth.Text, CommValue.AUTH_VALUE_FALSE);
            }
            catch (Exception ex)
            {
                ErrLogger.MakeLogger(ex);
