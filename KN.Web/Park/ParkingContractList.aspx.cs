...
{
    public partial class ParkingContractList : BasePage
    {
        ...
        protected void lvActMonthParkingCardList_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            {
                var ltTenantName = (Literal)iTem.FindControl("ltTenantName");
                ltTenantName.Text = TextLib.TextCutString(drView["Tenant_Name"].ToString(),35,"..");
            }

            if (!string.IsNullOrEmpty(drView["Tax_Code"].ToString()))
            {
                var ltTaxCode = (Literal)iTem.FindControl("ltTaxCode");
                ltTaxCode.Text = TextLib.StringDecoder(drView["Tax_Code"].ToString());
            }

            if (!string.IsNullOrEmpty(drView["Tax_Address"].ToString()))
            {
                var ltAddress = (Literal)iTem.FindControl("ltAddress");
                ltAddress.Text = TextLib.TextCutString(drView["Tax_Address"].ToString(), 35, "..");
            }

            var ltLessor = (Literal)iTem.FindControl("ltLessor");
            if ((!string.IsNullOrEmpty(drView["SubLessor"].ToString())) && drView["SubLessor"].ToString() =="Y")
            {

                ltLessor.Text = TextLib.StringDecoder("Sub Lessor");
            }
            else
            {
                ltLessor.Text = TextLib.StringDecoder("Lessor");
            }
        }
        ...
        protected void LoadRentDdl(DropDownList ddlParamNm, string strLangCd, string strGrpCd, string strMainCd)
        {
            ...
            {
                if (dr["CodeCd"].ToString().Equals(CommValue.RENTAL_VALUE_PARKING)) continue;
