...
{
    public partial class MonthParkingFeeUpdate : BasePage
    {
        ...
        private void LoadData()
        {
            ...
            dsReturn = ParkingMngBlo.SpreadSelectParkingFeeUpdate(ddlSearchYear.SelectedValue, ddlSearchMonth.SelectedValue, txtSearchRoom.Text,
        ...
        protected void lvActMonthParkingFeeList_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            if (e.Item.ItemType.Equals(ListViewItemType.DataItem))
            {
                ...
                if (!string.IsNullOrEmpty(drView["P_INVOICE"].ToString()))
                {
                    ...
                    txthfPInvoice.Text = TextLib.StringDecoder(drView["P_INVOICE"].ToString());
                }
                ...
                if (!string.IsNullOrEmpty(strCardNo) && !string.IsNullOrEmpty(strTagNo) && !string.IsNullOrEmpty(strCarNo))
                {
                    ...
                    else
                    {
                        ...
                        for (int intTmpI = 0; intTmpI < intLoopCnt; intTmpI++)
                        {
                            ...
                            //--Select Row from HoaDonParkingAPTReturn
        ...
        protected void lvActMonthParkingFeeList_ItemUpdating(object sender, ListViewUpdateEventArgs e)
        {
            try
            {
                // 세션체크
                ...
                string strCardNo = string.Empty;
                string strParkingYYYY = string.Empty;
                string strParkingMM = string.Empty;
                string strParkingCardFee = string.Empty;
                double dblCardFee = CommValue.NUMBER_VALUE_0_0;                
                ...
                TextBox txtHfYear = (TextBox)lvActMonthParkingFeeList.Items[e.ItemIndex].FindControl("txtHfYear");
                TextBox txtHfMM = (TextBox)lvActMonthParkingFeeList.Items[e.ItemIndex].FindControl("txtHfMM");
                TextBox txtFee = (TextBox)lvActMonthParkingFeeList.Items[e.ItemIndex].FindControl("txtFee");
                ...
                strCardNo = txtHfCardNo.Text;
                strParkingYYYY = txtHfYear.Text;
                strParkingMM = txtHfMM.Text;                
                dblCardFee = double.Parse(txtFee.Text.Replace(",", ""));

                //if (!string.IsNullOrEmpty(txtFee))
                //{
                //    object[] objReturn = ParkingMngBlo.ModifyParkingFeeApt(strRentCd, strRoomNo, strCardNo, strParkingYYYY, strParkingMM,
                //                                                                         dblCardFee, Session["CompCd"].ToString(), Session["MemNo"].ToString(), strInsMemIP);
                //}
                //else
                //{
                //    StringBuilder sbList = new StringBuilder();
