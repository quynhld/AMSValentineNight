...
{
    public partial class AccessList : BasePage
    {
        ...
        private void LoadData()
        {
            ...
            // KN_USP_COMM_SELECT_LOG_S02
            dsReturn = LogInfoBlo.SpreadAccessLogInfo(CommValue.BOARD_VALUE_PAGESIZE, Int32.Parse(hfCurrentPage.Value), hfStartDt.Value.Replace("-", ""), hfEndDt.Value.Replace("-", ""));
        ...
        protected void lvLogList_LayoutCreated(object sender, EventArgs e)
        {
            ...
            ((Literal)lvLogList.FindControl("ltTitleAccessSeq")).Text = TextNm["SEQ"];
            ((Literal)lvLogList.FindControl("ltTitleDate")).Text = TextNm["REGISTDATE"];
            ((Literal)lvLogList.FindControl("ltTitleMemNo")).Text = TextNm["MEMNO"];
            ((Literal)lvLogList.FindControl("ltTitleUserId")).Text = TextNm["USERID"];
            ((Literal)lvLogList.FindControl("ltTitleInsIP")).Text = TextNm["IP"];
        }
        ...
        protected void lvLogList_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            if (e.Item.ItemType.Equals(ListViewItemType.DataItem))
            {
                ...
                if (!string.IsNullOrEmpty(drView["AccessSeq"].ToString()))
                {
                    Literal ltAccessSeq = (Literal)iTem.FindControl("ltAccessSeq");
                    ltAccessSeq.Text = drView["AccessSeq"].ToString();

                    TextBox txtHfSeq = (TextBox)iTem.FindControl("txtHfSeq");
                    txtHfSeq.Text = drView["AccessSeq"].ToString();
                }
                ...
                if (!string.IsNullOrEmpty(drView["MemNo"].ToString()))
                {
                    Literal ltMemNo = (Literal)iTem.FindControl("ltMemNo");
                    ltMemNo.Text = drView["MemNo"].ToString();
                }

                if (!string.IsNullOrEmpty(drView["UserId"].ToString()))
                {
                    Literal ltUserId = (Literal)iTem.FindControl("ltUserId");
                    ltUserId.Text = drView["UserId"].ToString();
                }

                if (!string.IsNullOrEmpty(drView["InsIP"].ToString()))
                {
                    Literal ltInsIP = (Literal)iTem.FindControl("ltInsIP");
                    ltInsIP.Text = drView["InsIP"].ToString();
                }
            }
        }
        ...
        protected void lvLogList_ItemCreated(object sender, ListViewItemEventArgs e)
        {
            try
            {
                ...
                if (e.Item.ItemType.Equals(ListViewItemType.EmptyItem))
                {
                    ...
                    ((Literal)e.Item.FindControl("ltTitleAccessSeq")).Text = TextNm["SEQ"];
                    ((Literal)e.Item.FindControl("ltTitleDate")).Text = TextNm["REGISTDATE"];
                    ((Literal)e.Item.FindControl("ltTitleMemNo")).Text = TextNm["MEMNO"];
                    ((Literal)e.Item.FindControl("ltTitleUserId")).Text = TextNm["USERID"];
                    ((Literal)e.Item.FindControl("ltTitleInsIP")).Text = TextNm["IP"];
            ...
            try
            {
                ...
                for (int intTmpI = 0; intTmpI < lvLogList.Items.Count; intTmpI++)
                {
                    ...
                    if (chkLogSeq.Checked)
                    {
                        ...
                        // KN_USP_COMM_DELETE_LOG_M01
                        LogInfoBlo.RemoveAccessLogInfo(intLogSeq);
                        intTmpCnt++;
                    }
                }
