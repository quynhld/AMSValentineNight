<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RDPopupTenantTrialAPT.aspx.cs" Inherits="KN.Web.Common.RdPopup.RDPopupTenantTrialAPT" %>
...
<head id="Head1" runat="server">
 ...
 <body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="height:700px; width:1000px;">
        ...
        <script language="javascript" id="clientEventHandlersJS" type="text/javascript">
            ...
            Rdviewer.FileOpen('<%=NOW_DOMAIN%>/Common/Mrd/<%=strMRDFile %>', '/rv  FeeTy[<%=strDatum0 %>] FeeTyDt[<%=strDatum1 %>] Period[<%=strDatum2 %>] PeriodE[<%=strDatum3 %>]  StartDt[<%=strDatum4 %>] EndDt[<%=strDatum5 %>] PaymentCd[<%=strDatum0 %>] TenantNm[<%=strDatum7 %>] RoomNo[<%=strDatum6 %>] PaidCd[<%=strDatum8 %>] UserNm[<%=Session["MemNm"].ToString()%>]');
        </script>
    </body>
</html>