<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PopupCompAirConFindList.aspx.cs" Inherits="KN.Web.Common.Popup.PopupCompAirConFindList" %>
...
<!--    //
    function fnPopupConfirm(strRoom, strUserSeq,strComNm,strRentCd,strSquare)
    {
        ...
        var strReturnSquare = document.getElementById("<%=HfReturnSquare.ClientID%>").value;        
        ...
        opener.document.getElementById(strReturnSquare).value = strSquare;
        opener.callBack(strComNm, strRentCd, strRoom, strUserSeq, strSquare);
        self.close();
    }
...
<div style="text-align:center;overflow-y:scroll;height:500px;width:570px;">
    <asp:ListView ID="lvCompList" runat="server" ItemPlaceholderID="iphItemPlaceHolderId" OnItemDataBound="lvCompList_ItemDataBound"
     OnLayoutCreated="lvCompList_LayoutCreated" OnItemCreated="lvCompList_ItemCreated">
        ...
        <ItemTemplate>
            <tr style="background-color:#FFFFFF;cursor:pointer;" onmouseover="this.style.backgroundColor='#E4EEF5'" onmouseout="this.style.backgroundColor='#FFFFFF'" onclick='javascript:fnPopupConfirm("<%#Eval("RoomNo")%>", "<%#Eval("UserSeq")%>", "<%#Eval("UserNm")%>", "<%#Eval("RentCd")%>", "<%#Eval("RentLeasingArea")%>");'>
...
<asp:HiddenField ID="HfReturnSquare" runat="server"/>
