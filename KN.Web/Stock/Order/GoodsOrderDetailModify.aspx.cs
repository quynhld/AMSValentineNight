...
namespace KN.Web.Stock.Order
{
    ...
    public partial class GoodsOrderDetailModify : BasePage
    {
        ...
        protected void LoadExchageDate()
        {
            ...
            // dtReturn = ExchangeMngBlo.WatchExchangeRateLastInfo();
                    ...
                    if (isMemoSendOk)
                    {
                        ...
                        if (dtChargeReturn != null)
                        {
                            if (dtChargeReturn.Rows.Count > 0)
                            {
                                ...
                                // 구매 담당자 결제 요청 메모 발송
                                MemoWriteUtil.RegistrySendMemo(TextNm["PURCHASE"] + " " + TextNm["REQUEST"], MemoFormLib.MakeOrderChargeForm(strChargeMemNm, strOrderSeq), strChargeMemNo, Session["MemNo"].ToString(), strIP);
                                // 출고 최하위 담당자 처리
                                MemoWriteUtil.RegistrySendMemoDetail(strChargeMemNo, Session["MemNo"].ToString());
                            }
                        }
                    }
