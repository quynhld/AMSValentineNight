...
{
    public partial class TenantModify : BasePage
    {
        ...
        protected void LoadData()
        {
            ...
            if (string.IsNullOrEmpty(txtHfUserSeq.Text))
            {
                ...
                if (dsReturn != null)
                {
                    ...
                    // KN_USP_RES_SELECT_USERINFO_S04
                    dsViewReturn = ResidentMngBlo.WatchRentUserView(txtHfUserSeq.Text, txtHfRentCd.Text, Int32.Parse(txtHfRentSeq.Text), Session["LangCd"].ToString());
            ...
            {
                // KN_USP_RES_SELECT_USERINFO_S04
                DataSet dsReturn = ResidentMngBlo.WatchRentUserView(txtHfUserSeq.Text, txtHfRentCd.Text, Int32.Parse(txtHfRentSeq.Text), Session["LangCd"].ToString());
        ...
        protected void BindMainInfo(DataTable dtParam)
        {
            ...
            if (dtParam.Rows.Count > CommValue.NUMBER_VALUE_0)
            {
                ...
                {
                    txtKsystemCode.Text = TextLib.StringDecoder(dtParam.Rows[0]["KSys_CustCd"].ToString());
                }
            }
        }
            ...
            try
            {
                if (chkSameLessor.Checked)
                {
                    ...
                    if (!string.IsNullOrEmpty(txtHfRentCd.Text) && !string.IsNullOrEmpty(txtHfRentSeq.Text))
                    {
                        ...
                        // KN_USP_RES_SELECT_RENTINFO_S05
                        dtReturn = ResidentMngBlo.WatchTenantInfo(txtHfRentCd.Text, Int32.Parse(txtHfRentSeq.Text));
                        ...
                        if (dtReturn != null)
                        {
                            if (dtReturn.Rows.Count > 0)
                            {
                                ...
                                txtHfFloor.Text = dtReturn.Rows[0]["FloorNo"].ToString();
                                ...
                                txtTaxCd.Text = dtReturn.Rows[0]["LandloadTaxCd"].ToString();
                                txtTaxAddr.Text =  TextLib.StringDecoder(dtReturn.Rows[0]["LandloadAddr"].ToString());
                                txtTaxDetAddr.Text =  TextLib.StringDecoder(dtReturn.Rows[0]["LandloadDetAddr"].ToString());
                            }
                        }
                    }
                }
        ...
        protected void lnkbtnModify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    strOccuptionDt = hfOccupationDt.Value.Replace("-", "").Replace(".", "");
                //}

                if (!string.IsNullOrEmpty(txtBirthDt.Text))
                {
                    strBirthDt = txtBirthDt.Text.Replace("-", "").Replace(".", "");
                }

                if (!string.IsNullOrEmpty(txtOccupationDt.Text))
                {
