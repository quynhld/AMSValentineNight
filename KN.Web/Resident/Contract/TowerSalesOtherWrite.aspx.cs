...
namespace KN.Web.Resident.Contract
{
    ...
    public partial class TowerSalesOtherWrite : BasePage
    {
        ...
        protected void ddlTerm_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                txtSumRentUSDNo.CssClass = "bgType1";
                txtSumRentUSDNo.ReadOnly = CommValue.AUTH_VALUE_TRUE;
                txtSumRentVNDNo.Text = string.Empty;
                txtSumRentVNDNo.CssClass = "bgType1";
                txtSumRentVNDNo.ReadOnly = CommValue.AUTH_VALUE_TRUE;
                txtSumDepositUSDNo.ReadOnly = CommValue.AUTH_VALUE_FALSE;
        ...
        protected void txtSumRentVNDNo_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                // $에서 VND로 변환하는 부분
                txtSumRentUSDNo.Text = ((double.Parse(txtSumRentVNDNo.Text) / double.Parse(txtExchangeRate.Text))).ToString("##0.##");
            }
            else
            {
                txtSumRentUSDNo.Text = string.Empty;
        ...
        protected void imgbtnAdd_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
               ...
               // DataTable dtRentalFee;
                foreach (DataRow row in dtRentalFee.Rows)
                {
                    if (ReferenceEquals(row["ID"], txtHfRentFeeSeq.Text))
                    {
                        //row["SDate"] = txtRentalFeeStartDt.Text.Replace("-", "");
                        //row["EDate"] = txtRentalFeeEndDt.Text.Replace("-", "");
                        //row["VND"] = txtRentalFeeExcRate.Text;
                        //row["USD"] = txtRentalFeeExpAmt;
                    }
                    else
                    {
                        //dtRentalFee.Rows.Add(txtHfRentFeeSeq.Text, txtRentalFeeStartDt.Text.Replace("-", ""), txtRentalFeeEndDt.Text.Replace("-", ""), dblRentalFeeExpAmt, dblRentalFeeExcRate);
                    }
                    return;
                }
               ...
               // DataTable dtTmpRentalFee = ContractMngBlo.RegistryTempRentFeeInfo(txtHfRentCd.Text, intRentFeeTmpSeq, intRentFeeSeq, Session["CompCd"].ToString(), Session["MemNo"].ToString(), hfRentalFeeStartDt.Value.Replace("-", ""),
                ...
                //dtRentalFee = ContractMngBlo.SpreadTempRentFeeInfo(Int32.Parse(txtHfRentFeeTmpSeq.Text), Session["CompCd"].ToString(), Session["MemNo"].ToString());
        ...
        protected void lnkbtnWrite_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                riDs.R_PAYCYCLE = int.Parse(txtPayTermMonth.Text);
                riDs.R_PAYCYCLE_TYPE = ddlRPaymentCycle.SelectedValue;
                riDs.R_S_PAY_DATE = txtRSIsueDate.Text.Replace("-","");
                riDs.M_PAYCYCLE = int.Parse(txtMPayCycle.Text);
                riDs.M_PAYCYCLE_TYPE = ddlMPaymentCycle.SelectedValue;
                riDs.M_S_PAY_DATE = txtMSisueDate.Text.Replace("-", "");
                ...
                //{
                //    riDs.InitDay = CommValue.NUMBER_VALUE_0;
                //}
                //else
                //{
                //    riDs.InitDay = Int32.Parse(txtInitMMMngDay.Text);
                //}

                //riDs.InitMMMngDt = hfInitMMMngDt.Value.Replace("-", "");

                //if (string.IsNullOrEmpty(txtInitPerMMMngVND.Text))
                //{
                //    riDs.InitMMMngVNDNo = CommValue.NUMBER_VALUE_0_0;
                //}
                //else
                //{
                //    riDs.InitMMMngVNDNo = double.Parse(txtInitPerMMMngVND.Text);
                //}

                //if (string.IsNullOrEmpty(txtInitPerMMMngUSD.Text))
                //{
                //    riDs.InitMMMngUSDNo = CommValue.NUMBER_VALUE_0_0;
                //}
                //else
                //{
                //    riDs.InitMMMngUSDNo = double.Parse(txtInitPerMMMngUSD.Text);
                //}

                //if (string.IsNullOrEmpty(txtPerMMMngVND.Text))
                //{
                //    riDs.MMMngVNDNo = CommValue.NUMBER_VALUE_0_0;
                //}
                //else
                //{
                //    riDs.MMMngVNDNo = double.Parse(txtPerMMMngVND.Text);
                //}

                //if (string.IsNullOrEmpty(txtPerMMMngUSD.Text))
                //{
                //    riDs.MMMngUSDNo = CommValue.NUMBER_VALUE_0_0;
                //}
                //else
                //{
                //    riDs.MMMngUSDNo = double.Parse(txtPerMMMngUSD.Text);
                //}
