...
{
    public partial class TowerSalesList : BasePage
    {
        ...
        private void InitControls()
        {
            ...
            CommCdDdlUtil.MakeEtcSubCdDdlTitle(ddlConclusion, Session["LangCd"].ToString(), CommValue.ETCCD_VALUE_CONTSTEP);
        ...
        private void LoadData()
        {
            ...
            // KN_USP_RES_SELECT_RENTINFO_S00
            dsReturn = ContractMngBlo.SpreadRentInfo(CommValue.BOARD_VALUE_PAGESIZE, Int32.Parse(hfCurrentPage.Value), hfRentCd.Value, ddlKeyCd.Text, txtKeyWord.Text, ddlConclusion.SelectedValue, Session["LangCd"].ToString());
        ...
        protected void lvRentList_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            if (e.Item.ItemType.Equals(ListViewItemType.DataItem))
            {
                ...
                if (!string.IsNullOrEmpty(drView["ContractNm"].ToString()))
                {
                    ...
                    ltInsTenant.Text = TextLib.TextCutString(TextLib.StringDecoder(drView["ContractNm"].ToString()), 40, "...");
                }

                if (!string.IsNullOrEmpty(drView["RentStartDt"].ToString()) && !string.IsNullOrEmpty(drView["RentEndDt"].ToString()))
                {
                    StringBuilder sbDuration = new StringBuilder();

                    if (!string.IsNullOrEmpty(drView["RentStartDt"].ToString()))
                    {
                        sbDuration.Append(drView["RentStartDt"].ToString().Substring(0, 4));
                        sbDuration.Append(".");
                        sbDuration.Append(drView["RentStartDt"].ToString().Substring(4, 2));
                        sbDuration.Append(".");
                        sbDuration.Append(drView["RentStartDt"].ToString().Substring(6, 2));
                    }

                    sbDuration.Append(" ~ ");

                    if (!string.IsNullOrEmpty(drView["RentEndDt"].ToString()))
                    {
                        sbDuration.Append(drView["RentEndDt"].ToString().Substring(0, 4));
                        sbDuration.Append(".");
                        sbDuration.Append(drView["RentEndDt"].ToString().Substring(4, 2));
                        sbDuration.Append(".");
                        sbDuration.Append(drView["RentEndDt"].ToString().Substring(6, 2));
                    }
                ...
                if (!string.IsNullOrEmpty(drView["ContStepNm"].ToString()))
                {
                    Literal ltContStep = (Literal)e.Item.FindControl("ltContStep");
                    ltContStep.Text = drView["ContStepNm"].ToString();
                }
        ...
        protected void lnkbtnExcelReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // KN_USP_RES_SELECT_RENTINFO_S02
                // KN_SCR_SELECT_CONTRACT_EXCEL_LIST
                dtReturn = ContractMngBlo.SpreadRentInfoExcelView(hfRentCd.Value, "", "", "");
                ...
                var strTitle = "<p align='center'><font size='4'><b>" + Master.TITLE_NOW.ToString() + "</b></font></p>";
