...
{
    public partial class ReceiptPrintOutForAPT : BasePage
    {
        ...
        protected void InitControls()
        {
           ...
           // MultiCdDdlUtil.MakeMemCompCdNoTitle(ddlCompNo, "00000000");
            ...
            lnkbtnIssuing.Text = "Printing Receipt";
        ...
        protected void lnkbtnIssuing_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (var i = CommValue.NUMBER_VALUE_0; i < lvPrintoutList.Items.Count; i++)
                {
                    ...
                    InvoiceMngBlo.UpdatingRefPrintNoForAPT(refSeq, hfRentCd.Value, refPrintNo, PayDt);
                    intCheckRow++;
                }               
                 ...
                 if (intCheckRow > CommValue.NUMBER_VALUE_0)
                {
                    ...
                    sbResult.Append("window.open('/Common/RdPopup/RDPopupReciptPaymentAPT.aspx?Datum0=" + refPrintNo + "&Datum1=" + billCd + "&Datum2=" + paydt + "&Datum3=&Datum4=', 'ParkingFee', 'status=no, resizable=no, width=740, height=900, left=100,top=100, scrollbars=no, menubar=no, toolbar=no, location=no');");
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ReceiptVoucher", sbResult.ToString(), CommValue.AUTH_VALUE_TRUE);
                }
                else
                {
                    // 화면 초기화
