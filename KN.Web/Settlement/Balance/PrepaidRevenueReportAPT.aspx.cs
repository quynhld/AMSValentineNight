...
{
    public partial class PrepaidRevenueReportAPT : BasePage
    {
        ...
        protected void lnkbtnIssuing_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var paidDt = txtSearchDt.Text.Trim().Replace("-", "");
                var paidDtE = txtSearchDtE.Text.Trim().Replace("-", "");
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "PreviewPrint", "fnOccupantList('" + period + "','" + periodE + "','" + paidDt + "','" + paidDtE + "');", CommValue.AUTH_VALUE_TRUE);
            }
            catch (Exception ex)
            {
                ErrLogger.MakeLogger(ex);
        ...
        protected void lnkbtnExcelReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AuthCheckLib.CheckSession();
                var period = txtPeriod.Text.Trim().Replace("-", "");
                var periodE = txtPeriod.Text.Trim().Replace("-", "");
                var paidDt = txtSearchDt.Text.Trim().Replace("-", "");
                var paidDtE = txtSearchDtE.Text.Trim().Replace("-", "");

                var dtReturn = ContractMngBlo.SpreadPrepaidRevenue(period,periodE,paidDt,paidDtE);
                var fileNm = Server.UrlEncode(Master.TITLE_NOW).Replace("+", " ");
                GenerateExcel(dtReturn,fileNm,fileNm);
        ...
        public void GenerateExcel(DataTable tbl, string excelSheetName, string fileName)
        {
            using (var pck = new ExcelPackage())
            {
                ...
                //using (var col = ws.Cells[2, 3, 2 + tbl.Rows.Count, 3])
                //{
                //    col.Style.Numberformat.Format = "MM/yyyy";
                //    col.Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                //}
                Response.Clear();
