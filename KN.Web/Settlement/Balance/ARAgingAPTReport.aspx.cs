...
{
    public partial class ARAgingAPTReport : BasePage
    {
        ...
        protected void lnkbtnIssuing_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ////var rentcd = ddlInsRentCd.SelectedValue.ToString();
                //var period = txtPeriod.Text.Trim().Replace("-", "");
                //var periodE = txtPeriodE.Text.Trim().Replace("-", "");
                //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "PreviewPrint", "fnOccupantList('" + rentcd + "','" + period + "','" + periodE + "');", CommValue.AUTH_VALUE_TRUE);
            }
            catch (Exception ex)
            {
                ErrLogger.MakeLogger(ex);
            ...
            try
            {
                ...
                var startDt = txtPeriod.Text.Trim().Replace("-", "");
                var endDt = txtPeriodE.Text.Trim().Replace("-", "");

                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "PreviewPrint", "fnOccupantList('" + feeTy + "','" + startDt + "','" + endDt + "','" + roomNo + "');", CommValue.AUTH_VALUE_TRUE);
        ...
        protected void lnkbtnExcelReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var dtSource = BalanceMngBlo.SelectExcelAPTAgingReport(ddlFeeType.SelectedValue, strStartDt, strEndDt, txtRoomNo.Text);

                if (dtSource.Tables[0].Rows.Count <= 0)
                {
                    return;
                }                
                var FeeTyNm = ddlFeeType.SelectedValue == "" ? "" : "-" + ddlFeeType.SelectedItem.ToString();

                var fileName = Server.UrlEncode("AR Aging Report").Replace("+", " ") + FeeTyNm + " - " + strEndDt;
        ...
        public void GenerateExcel(DataTable tbl, string excelSheetName, string fileName)
        {
            using (var pck = new ExcelPackage())
            {
                ...
                tbls.ShowTotal = true;
                //Set Sum 
                tbls.Columns[5].TotalsRowFunction = RowFunctions.Sum;
                tbls.Columns[6].TotalsRowFunction = RowFunctions.Sum;
                ...
                using (var col = ws.Cells[2, 5, 2 + tbl.Rows.Count, 9])
                {
