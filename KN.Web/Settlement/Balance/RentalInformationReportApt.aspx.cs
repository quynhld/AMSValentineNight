...
{
    public partial class RentalInformationReportApt : BasePage
    {
        ...
        protected void lnkbtnIssuing_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "PreviewPrint", "fnOccupantListCohabitant('" + rentcd + "','" + period + "','" + periodE + "');", CommValue.AUTH_VALUE_TRUE);
            }
            catch (Exception ex)
            {
                ErrLogger.MakeLogger(ex);
        ...
        protected void lnkbtnExcelReport_Click(object sender, EventArgs e)
        {
            try
            {
                // 세션체크
                AuthCheckLib.CheckSession();
                var strStartDt = txtPeriod.Text.Trim().Replace("-", "");
                var strEndDt = txtPeriod.Text.Trim().Replace("-", "");
                var strRentCd = ddlInsRentCd.SelectedValue;

                var dtReturn = BalanceMngBlo.SelectExcelRentalInformationReport(strRentCd, strStartDt, strEndDt);
                if (dtReturn.Tables[0].Rows.Count <= 0)
                {
                    return;
                }

                var fileNm = Server.UrlEncode(Master.TITLE_NOW).Replace("+", " ");
                GenerateExcel(dtReturn.Tables[0], fileNm, fileNm);
