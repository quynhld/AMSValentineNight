...
{
    public partial class TenantLedgerDetailReport : BasePage
    {
        ...
        protected void InitControls()
        {
            ...
            txtPeriod.Text = DateTime.Now.ToString("s").Substring(0, 7);
            ...
            CommCdDdlUtil.MakeSubCdDdlUserTitle(ddlPaidCd, Session["LangCd"].ToString(), CommValue.COMMCD_VALUE_ACCOUNT, CommValue.ACCOUNT_VALUE_PAYMENT_METHOD, TextNm["ENTIRE"]);
        ...
        {      
            ...
            try
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "PreviewPrint", "fnOccupantList('" + feeTy + "','" + feeTyDt + "','" + period + "','" + periodE + "','" + paydt + "','" + paydtE + "','" + roomNo + "','" + userNm + "','" + paidCd + "');", CommValue.AUTH_VALUE_TRUE);
            }
            catch (Exception ex)
            {
                ErrLogger.MakeLogger(ex);
        ...
        protected void txtRoomNo_TextChanged(object sender, EventArgs e)
        {
            try
            {
                string strRoomNo = txtRoomNo.Text;
                var strFPeriod = string.Empty;

                txtPeriod.Text = "";

                // KN_USP_RES_SELECT_VESTINGDATE_S00
                DataTable dtVestingDt = RoomMngBlo.SelectAptVestingDate(strRoomNo);

                if (dtVestingDt != null)
                {
                    if (dtVestingDt.Rows.Count > CommValue.NUMBER_VALUE_0)
                    {

                        strFPeriod = dtVestingDt.Rows[0]["LastKeyDt"].ToString().Substring(0, 6);
                        //strFPeriod = dtVestingDt.Rows[0]["LastKeyDt"].ToString().Substring(0, 4) + "-" + dtVestingDt.Rows[0]["LastKeyDt"].ToString().Substring(4, 2);
                           
               
                            // 해당 호실이 있는 경우 
                        
                            StringBuilder sbList = new StringBuilder();
                            sbList.Append("document.getElementById('" + txtPeriod.ClientID + "').value = " + strFPeriod + ";");  
                            //sbList.Append("document.getElementById('" + txtPeriod.ClientID + "').focus();");

                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "NoDateAlert", sbList.ToString(), CommValue.AUTH_VALUE_TRUE);
                     }
                        else
                        {
                            // 해당 호실이 없는 경우                          
                            ...
                            sbList.Append("document.getElementById('" + txtPeriod.ClientID + "').value = '';");
