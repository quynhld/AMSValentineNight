...
{
    public partial class ManuallySettingPrintList_Test : BasePage
    {
        ...
        protected void InitControls()
        {
            ...
            hfreqDate.Value = txtReqDt.Text.Replace("-", "").Replace(".", "");
        ...
        protected void LoadData()
        {
            ...
            //KN_USP_SELECT_DEBIT_LIST_S00
            var dtReturn = MngPaymentBlo.SpreadManuallyPrintingList(rentCode,roomNo,payCode,nm,strStartDt, strEndDt, ddlPrintYN.SelectedValue);            
        ...
        private void MakeUsdVndDdl()
        {
            ddlUV.Items.Clear();
            ddlUV.Items.Add(new ListItem("VND", "0001"));
            ddlUV.Items.Add(new ListItem("USD", "0002"));
            ddlUV.Items.Add(new ListItem("USD&VND", "0003"));
            ddlUV.Items.Add(new ListItem("Out Side USD", "0004"));
        ...
        {           
            foreach (var t in lvPrintoutList.Items)
            {
                ...
                var rentCd = ((TextBox)t.FindControl("txtDebitCode")).Text;
                var feeTyNm = ((TextBox)t.FindControl("txthfFeeTypeCode")).Text;
                var bundleSeq = ((TextBox)t.FindControl("txtRef_Seq")).Text;
                var invoice = ((Literal)t.FindControl("ltInvoice")).Text.Trim();
                if(string.IsNullOrEmpty(invoice))
                {
                    MngPaymentBlo.CancelPrintingList(rentCd, userSeq, feeTyNm, roomNo, bundleSeq);    
                }
                else
                {                    
                    CloseLoading();
                    Aleart("Cannot cancel debit already issue !");                    
                    return;
                }                            
            }
            LoadData();
            Aleart("Cancel successful !");
        }
        ...
        protected void lnkPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                InvoiceMngBlo.UpdatingNullPrintBundleNo(hfRentCd.Value);
                ...
                for (var i = CommValue.NUMBER_VALUE_0; i < lvPrintoutList.Items.Count; i++)
                {
                    ...
                    InvoiceMngBlo.UpdatingPrintBundleNo(refSeq,refPrintBundleNo);
                    intCheckRow++;
                }
