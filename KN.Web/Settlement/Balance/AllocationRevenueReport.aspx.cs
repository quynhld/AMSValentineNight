...
{
    public partial class AllocationRevenueReport : BasePage
    {
        ...
        protected void lnkbtnExcelReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // 세션체크
                AuthCheckLib.CheckSession();               
                string strRoomNo = txtInsRoomNo.Text;
                string strPeriod = ddlSearchYear.SelectedValue.ToString();

                // KN_USP_MNG_SELECT_ACCOUNTSINFO_S01
                var dtSource = BalanceMngBlo.SelectExcelAllocationRevenueReport(strRoomNo, ddlSearchYear.SelectedValue);
                ...
                var sheetname = strPeriod;

                var fileName = Server.UrlEncode("Allocation For Monthly Revenue ").Replace("+", " ") + strPeriod;
        ...
        public void GenerateExcel(DataTable tbl, string excelSheetName, string fileName)
        {
            using (var pck = new ExcelPackage())
            {
                ...
                ws.SelectedRange[1, 1, 1, 11].Merge = true;
                ws.SelectedRange[2, 1, 2, 11].Merge = true;
                ...
                ws.Cells["I3:V3"].AutoFitColumns();

                using (var col = ws.Cells[4, 5, 4 + tbl.Rows.Count, 5])
                {
                ...
                using (var col = ws.Cells[4, 9, 4 + tbl.Rows.Count, 21])
                {
                    col.Style.Numberformat.Format = "#,##0";
                    col.Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                }
